...
test('validate \'Add to cart\' Button enable when product is avalibale', async ({ page }) => {
    const catalog = new CatalogPage(page);
...
test('validate \'Add to cart\' Button disable when product is NOT avalibale', async ({ page }) => {
    const catalog = new CatalogPage(page);
    await test.step('Validate \'Add to cart\' Button', async () => {
